function config() {
    ...
    var result = karate.callSingle('classpath:demo/header/rew3-header.feature', config);
    // and it sets a variable called 'authInfo' used in headers-single.feature
    print(result.response.token_type + ' ' + result.response.access_token + '--------------------------------------------------------')
    // config.authInfo = { authTime: result.time, authToken: result.token };
    // print(config.authInfo)
    config.rew3Token = result.response.token_type + ' ' + result.response.access_token;
